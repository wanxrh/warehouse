			...
			<div class="tab-content">
				...
				<form id="form" action="" method="post" class="form-horizontal form-center">
					...
					<div class="form-item cf toggle-title">
						...
						经纬度：
						<input id="lonlat" name="latitude" type="text" value="<?php echo $store['latitude'];?>" ><a href="javascript:void(0);" id="dianji">获取经纬度</a>
						...
						<div class="controls">
							<label class="textarea">
								<select name="open_id" style="width: 350px;">
									...
									<option value="<?php echo $item['openid'];?>" <?php if($store['open_id']==$item['openid']):?>selected<?php endif;?>><?php echo $item['remark'].'：'.$item['openid'];?></option>
									<?php endforeach;?>
								</select>
							</label>
						</div>
					...
					<div class="form-item cf toggle-title copy-node">
						...
						<div class="per">
							<?php foreach ($store_goods as $key=>$item):?>
							<?php if($key==0):?>
								<div class="controls">
									<label class="textarea">
										<select name="goods_id[]" style="width: 400px;">
											<?php foreach($goods as $val):?>
											<option value="<?php echo $val['id'];?>" <?php if($item['goods_id']==$val['id']):?>selected<?php endif;?>><?php echo $val['title'];?></option>
											<?php endforeach;?>
										</select>
									数量：<input type="text" name="number[]"  value="<?php echo $item['number'];?>">  <a href="javascript:void(0);" class="clone">添加</a>
									</label>
								</div>
							<?php else:?>
									<div class="controls del-clone">
										<label class="textarea">
											<select name="goods_id[]" style="width: 400px;">
												<?php foreach($goods as $val):?>
													<option value="<?php echo $val['id'];?>" <?php if($item['goods_id']==$val['id']):?>selected<?php endif;?>><?php echo $val['title'];?></option>
											...
											数量：<input type="text" name="number[]" value="<?php echo $item['number'];?>">  <a href="javascript:void(0);" class="clone2">删除</a>
										</label>
									</div>
							<?php endif;?>
							<?php endforeach;?>
...
<script type="text/javascript">
            $(function() {
               $("#city_4").citySelect({
                    prov: "<?php echo $store['provinces'];?>",
                    city: "<?php echo $store['city'];?>",
                    dist: "<?php echo $store['county'];?>",
                    nodata: "none"
                });
	...
	<script type="text/javascript">
		...
		//创建点坐标
		var point = new BMap.Point(<?php echo $store['latitude'];?>);
		//初始化地图，设置中心点坐标和地图级别
...
// 创建标注
		...
		//标注点数组
		var markerArr = [{title:"南宁市安启汽车服务公司",content:"长湖路36号金湖富地广场1楼(门市部)8楼(办公室)<br/>联系电话：0771-5518232",point:"108.381981|22.828751",isOpen:1,icon:{w:21,h:21,l:0,t:0,x:6,lb:5}}
		];
		//********************************************监听标注事件
		//点击事件
