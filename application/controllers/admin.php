...
class Admin extends BaseController {
	...
	public function addQrcode(){
		...
		$agent_url = 'http://pan.baidu.com/share/qrcode?url='.$this->config->base_url().'agent/activat/'.$id.'/'.$ActivatCode;
    ...
    public function editProduct(){
        if(IS_POST){
            ...
            $row = $this->AdminModel->update('shop_source',array('id'=>$id),$param);
            if(!$row){
                ajaxError('操作失败！');
            }
            ajaxSuccess('操作成功',array('url'=>'/admin/product'));
