...
class MallModel extends BaseModel {
	...
	public function goodsList(){
		$this->db->order_by('sort','ASC');
		return $this->db->select('id,title,price,cover')->where(array('is_show'=>1,'reserve'=>0))->limit(20)->order_by('sale_count','desc')->get('shop_goods')->result_array();
	}
	...
	public function searchGoodsLists($search_key,$cate,$order_key,$order_type){
		...
		$this->db->select('id,title,price,cover')->where(array('is_show'=>1,'reserve'=>0));
    ...
    public function reserveList($search_key){
        if($search_key){
            $this->db->like('title',$search_key);
        }
        $this->db->order_by('sort','ASC');
    ...
    public function get_store_info($user_id){
        $this->db->where('id',$user_id);
        $this->db->select('*');
        return $this->db->get('shop_daili_store')->row_array();
    }
    public function get_store_goods($id){
        $this->db->where('daili_id',$id);
