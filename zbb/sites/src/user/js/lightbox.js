 ...
 * @site wwww.yiban.com
 * @version 0.2
 * @date 2014-2-19 //$(".lightbox").sklightbox();
 */
    ...
    $.fn.lightbox = function() {
        ...
        $this.find("img").off("click").on("click", function() {
            ...
            for (var i = 0; i < Allimg; i++) {
                ...
                _srt += '<img id="light_img" src="' + aResult[i].src + '"/>'; 
            }
            var _int = $(this).parent().find("img").index($(this));
        ...
        var showPopup = function(_int, Allimg, _srt) {
        ...
        //图片判断完成
        var loadComplete = function(_int, Allimg, imgdefereds) {
            _imgLength--;
            if (_imgLength == 0) {
                clickSwitch(_int, Allimg, imgdefereds);
            }
        ...
        //切换显示图片。
        var clickSwitch = function(_int, Allimg, imgdefereds) {
            $(".popup").show();
            for (var i = 0; i < Allimg; i++) {
                //显示相对应图片
                if (i != _int) {
        ...
        //重写弹出层大小。
        var resizePop = function(h, w, _top, _left) {
