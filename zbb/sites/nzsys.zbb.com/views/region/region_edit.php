                    ...
                    <form id="article_form" enctype="multipart/form-data" method="post">
                        <ul class="forminfo">
                            <li><label>分类名称<b>*</b></label>
                                <input name="region_name" type="text" class="dfinput" value="<?php echo $row['region_name'] ?>"  style="width:300px;" maxLength="100" onkeyup="value=value.replace(/[\d]/g,'') "onbeforepaste="clipboardData.setData('text',clipboardData.getData('text').replace(/[\d]/g,''))"/>
                            </li>
                            ...
                            <li><label>上级分类<b>*</b></label>
                                <select id="cate_id" class="querySelect" name="parent_id">
                                    ...
                                    <?php foreach ($region as $item): ?>
                                        <option value="<?php echo $item['region_id'] ?>" <?php if ($row['parent_id'] == $item['region_id']): ?>selected="selected"<?php endif; ?>><?php echo $item['region_name'] ?></option>
                                    <?php endforeach; ?>
                                </select>
                            </li>
                            ...
                            <li><label>排序<b>*</b></label>
                                <input name="sort_order" type="text" class="dfinput" value="<?php echo $row['sort_order'] ?>"  style="width:300px;" maxLength="3" onkeyup="this.value=this.value.replace(/\D/g,'')" onafterpaste="this.value=this.value.replace(/\D/g,'')"/>
                            </li>   
