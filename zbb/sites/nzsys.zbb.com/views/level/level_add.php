	...
	<body>
		<div class="u-bd-tt">
			...
			<ul >
				...
				<li><a href="/level/add">新增管理员权限</a></li>
			</ul>
		</div>
					...
					<form id="article_form" enctype="multipart/form-data" method="post">
						<ul class="forminfo">
							<li><label>管理员名称<b>*</b></label>
								<input name="user_name" type="text" class="dfinput" value=""  style="width:300px;" maxLength="100" />
							</li>

							<li id="all_checkbox"><label>选择权限<b>*</b></label>
								<?php foreach ($level as $key => $val): ?>
									<span style="float: left; width: 110px;">                           	
										<b style="color:#F00"><input  type="checkbox" value="<?php echo $val['level_id'] ?>" name="level_id[]" /><?php echo $val['level_name'] ?></b>
										<?php if (isset($val['child'])): ?>
										<?php foreach ($val['child'] as $k => $v): ?>
											<span><input  type="checkbox" value="<?php echo $v['level_id'] ?>" name="level_id[]" /><?php echo $v['level_name'] ?></span>
										<?php endforeach; ?>
										<?php endif;?>	
									</span> 
								<?php endforeach; ?>
