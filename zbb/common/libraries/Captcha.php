...
class Captcha {
	...
	public static $imageL = 120; // 验证码图片长
	public static $length = 4; // 验证码位数
	public static $bg = array (
			243,
			251,
			254 
	); // 背景
	...
	private static $_codeSet = '2346789abcdefghkmnpqrstuvwxyz';
	...
	public static function check($code, $id = '') {
		isset ( $_SESSION ) || session_start ();
		// 验证码不能为空
		if (! isset ( $_SESSION [self::$seKey] ) || empty ( $code ) || empty ( $_SESSION [self::$seKey] )) {
			return false;
		}
		...
		if (isset ( $secode ['code'] ) && strtolower ( $code ) == $secode ['code']) {
			return true;
		}
	...
	protected static function _writeNoise() {
		for($i = 0; $i < 10; $i ++) {
			...
			for($j = 0; $j < 5; $j ++) {
				// 绘杂点
				imagestring ( self::$_image, 5, mt_rand ( - 10, self::$imageL ), mt_rand ( - 10, self::$imageH ), self::$_codeSet [mt_rand ( 0, 27 )], 				// 杂点文本为随机的字母或数字
				$noiseColor );
			}
		}
	}
